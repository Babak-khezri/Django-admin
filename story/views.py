...
def highlite_create_view(request):
    if request.method == 'POST':
        ...
        if stories:
            ...
            image = form.cleaned_data['image'] if form.is_valid() else stories[0].image
            highlight = Highlight.objects.create(user=request.user,image=image,name=request.POST.get('name'))
            for story in stories:
